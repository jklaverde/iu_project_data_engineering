...
| `app/environment.py` | Pure functions (no I/O) turning a device's latest reading + its Cassandra-persisted threshold stats into role-appropriate signals: `metric_status`/`device_status` (ok/warning/critical, reusing Spark's own anomaly thresholds), `metric_ranges` (per-metric actual value + normal band + ceiling, for the UI's gauge), `air_quality_score`, `comfort_index`, `chronic_exposure_ratio`, `trend_direction`, and `metric_windows`/`rollup_metric_windows` (D34/D35 — project one metric out of agg_1m/agg_1h rows, or roll agg_1h up into day/week/month buckets, for the timeline charts and the out-of-range log). |
...
| `src/planner/SensorTimeline.tsx` | Full-width behavior-over-time panel (D34) — a metric selector plus five charts (minute/hour/day/week/month, `GET /api/sensors/{id}/timeline`), each with a shaded `markArea` over any span where that metric was outside the acceptable range. Exports the shared `METRICS`/`GRANULARITIES` lists and renders `BoundaryLog` below the chart grid using the same already-fetched data (no second request). |
