...
- FR-W7. A "Docs" tab (D36) lists and renders a subset of the project's own `.md` documentation
  (README, `docs/ARCHITECTURE.md`, `docs/PROJECT_STRUCTURE.md`, `docs/DEPLOYMENT.md` — via
  `GET /api/admin/docs`, `GET /api/admin/docs/{doc_id}`), bundled into the backend image at build
  time, so an administrator can read how the system was conceived and built without a repo
  checkout on the host. `REQUIREMENTS.md` itself, `docs/PROGRESS.md`, and
  `docs/TROUBLESHOOTING.md` stay in the repository as the project's own record but are not
  rendered in-app (D36).
...
| D36 | Admin "Docs" tab | A subset of the project's own `.md` documentation (root `README.md`, plus `docs/ARCHITECTURE.md`, `PROJECT_STRUCTURE.md`, `DEPLOYMENT.md`) is bundled into the backend image at build time (`backend/Dockerfile`) and served admin-only (`backend/app/routers/docs.py`, gated like `admin.alerts_router`) so an administrator can read how the system was conceived and built from inside the running app, no repo checkout needed. `REQUIREMENTS.md`, `docs/PROGRESS.md` and `docs/TROUBLESHOOTING.md` were deliberately dropped from the in-app catalog after the initial version included them — they stay in the repository as the project's own development record, but track process/status rather than the system an administrator needs to operate, so they were scoped out. `docs/internal/` is likewise excluded — gitignored, local-only working notes, not part of the project's record. Rendered client-side with `marked` (frontend NFR-10.1 justification in `frontend/README.md`), added to the dependency allowlist because the source documents contain real tables and nested lists a hand-rolled parser risks mis-rendering; it only ever parses this project's own committed documentation, never user input. |
